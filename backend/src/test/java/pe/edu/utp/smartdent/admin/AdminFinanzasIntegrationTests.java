...
@Transactional
class AdminFinanzasIntegrationTests {
    ...
    @Test
    void adminDebeConsultarReporteFinanciero() throws Exception {
        LoginResponse admin = loginService.iniciarSesion(new LoginRequest("admin@smartdent.com", "Admin123"));

        mockMvc.perform(get("/api/admin/reportes/finanzas")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + admin.token()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.ingresosMesActual").exists())
                .andExpect(jsonPath("$.costosVariablesMesActual").exists())
                .andExpect(jsonPath("$.gastosFijosMensuales").exists())
                .andExpect(jsonPath("$.mensual").isArray())
                .andExpect(jsonPath("$.demandaServicios").isArray());
    }

    @Test
    void odontologoNoDebeConsultarReporteFinancieroAdmin() throws Exception {
